// pages/englishLevel/englishLevel.js
const app = getApp()
Page({
   ...
   */
  data: {
    array: ['无','大学英语三级','成人英语三级', '大学英语四级', '大学英语六级','专业英语六级','专业英语八级'],
    index: 0,
    change: 'save',
    saveClickNum: 0,
  },
      ...
      if (app.globalData.allUserInfo !== null) {
        wx.request({
          ...
          data: {
            ...
            englishGrade: that.data.array[that.data.index]
          },
          header: {
  ...
  onLoad: function (options) {
    if (app.globalData.allUserInfo !== null) {
      if (app.globalData.allUserInfo.englishGrade !== null) {
        for (let i = 0; i < this.data.array.length; i++) {
          if (this.data.array[i] === app.globalData.allUserInfo.englishGrade) {
            this.setData({
              index: i
            })
          }
